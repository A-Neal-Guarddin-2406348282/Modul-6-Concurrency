...
Pada tugas ini, saya mempelajari cara kerja server TCP sederhana di Rust melalui fungsi [`handle_connection`](src/main.rs). Berdasarkan dokumentasi Rust, `TcpStream` merepresentasikan koneksi jaringan yang aktif, sedangkan `BufReader` membantu membaca data dari stream secara efisien baris demi baris.
...
### Kesimpulan
Saya memahami bahwa penanganan request lambat perlu dipikirkan dengan baik dalam desain web server. Refactoring ke model yang lebih efisien, seperti multi-threading atau async, diperlukan agar server tetap responsif saat melayani banyak request.
...
### Yang saya pelajari
- [`ThreadPool::build`](src/lib.rs) membuat sejumlah worker yang siap menerima tugas.
...
Pada bonus ini, saya melakukan refactoring kecil pada API thread pool dengan membuat [`ThreadPool::build`](src/lib.rs) sebagai pengganti [`ThreadPool::new`](src/lib.rs). Perubahan ini tidak mengubah perilaku utama program, tetapi membuat penamaan fungsi lebih jelas dan lebih sesuai dengan tujuan pembuatannya.

### Yang saya pelajari
- [`ThreadPool::build`](src/lib.rs) digunakan untuk membuat thread pool dengan jumlah worker tertentu.
- Logika pembuatan worker tetap sama seperti sebelumnya.
- [`ThreadPool::execute`](src/lib.rs) tetap digunakan untuk mengirim job ke worker.
- Di [src/main.rs](src/main.rs), server sekarang memanggil [`ThreadPool::build`](src/lib.rs) saat membuat pool.
...
### Kesimpulan
Bonus ini membantu saya memahami bahwa refactoring kecil pada API internal juga penting. Walaupun perilakunya sama, perubahan nama method dapat meningkatkan kejelasan dan keterbacaan kode.